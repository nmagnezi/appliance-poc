...
function iso2_generator_main() {
  ...
  log_info "${func_name}" "Copy (via skopeo) local registry: $REGISTRY_IMAGE to $POC_DIR/images/registry.tar:registry:2"
  skopeo copy docker://"$REGISTRY_IMAGE" docker-archive:"$POC_DIR"/images/registry.tar:registry:2
  log_info "${func_name}" "Copy (via skopeo) assisted-installer-agent: $ASSISTED_INSTALLER_AGENT_IMAGE to $POC_DIR/images/ose-agent-installer-node-agent"
  skopeo copy --all docker://"$ASSISTED_INSTALLER_AGENT_IMAGE" dir:"$POC_DIR"/images/ose-agent-installer-node-agent
  log_info "${func_name}" "Copy (via skopeo) assisted-service: $ASSISTED_INSTALLER_AGENT_IMAGE to $POC_DIR/images/ose-agent-installer-api-server"
  skopeo copy --all docker://"$ASSISTED_SERVICE_IMAGE" dir:"$POC_DIR"/images/ose-agent-installer-api-server
}
